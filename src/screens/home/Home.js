...
const Home = ({navigation}) => {
  ...
  const synchronize = async () => {
      ...
      await db.transaction(tx => {
        ...
        tx.executeSql(
          ...
          [
            ...
            'Offline Data',
            'Upload',
            'Data Uploading to Server',
            date,
            date,
          ],
          () => {
            console.log(
              'Offline Data Audit - Data Upload inserted successfully',
            );
          },
        );
      });
    ...
    let totalRecords = PostData.length;
    ...
    if (VerificationImage != null && VerificationImage != '') {
      processFiles(folderPath, VerificationImage, completedTasks, totalImages);
    }
    async function processFiles(
      ...
      if (files.length > 0) {
        ...
        for (const file of files) {
          ...
          completedTasks++;
          const progress2 = (completedTasks / totalImages) * 100;
    ...
    db.transaction(async tx => {
      tx.executeSql(
        'SELECT EntryDate AS entrydate,UserId AS userId,Username AS username,DeviceId AS mobileId,ApkVersion AS apk ,Module AS modulename, Actions AS action,Remarks AS remarks FROM MDB_Audit',
        [],
        async (tx, results) => {
          var len = results.rows.length;
          if (len > 0) {
            for (let i = 0; i < results.rows.length; ++i)
              await SendAuditData(results.rows.item(i));
          }
        },
      );
    });
  ...
  const SendAuditData = async AuditData => {
    try {
      await fetch(`${API_URL}/api/audit-trail/createLog`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const droptable = async => {
    try {
      ...
      db.transaction(async tx => {
        ...
        await tx.executeSql('DROP TABLE MDB_setting', [], (tx, results) => {
      ...
      db.transaction(async tx => {
        ...
        await tx.executeSql('DROP TABLE MDB_Audit', [], (tx, results) => {
          console.log('Table MDB_Audit Deleted successfully');
        });
      });
  ...
  return (
    <View style={gStyle.container[theme]}>
      <View style={[styles.container]}>
        ...
        />
        {sync == true ? (
          progress === 100 && progress2 == 100 && complete == true ? (
            <StandardAlert
              visible={visible}
              color={'green'}
