  ...
  const DownloadOffline = async () => {
    try {
      ...
      if (selected != null) {
        ...
        temp3 =
          'Region : ' +
          JSON.stringify(selected_region.name) +
          '/ ' +
          'EstateGroup : ' +
          JSON.stringify(selected_estate_group.name) +
          '/ ' +
          'Estate : ' +
          JSON.stringify(selected_estate.name) +
          '/ ' +
          'Afdeling : ' +
          JSON.stringify(selected_afdeling.name) +
          '/ ' +
          'Blok : ' +
                      ...
                      await db.transaction(tx => {
                        ...
                        tx.executeSql(
                          ...
                          [
                            ...
                            'Master Data',
                            'Download',
                            `User Downloaded New Blocks - ${temp3}`,
                            date,
                            date,
                          ],
                          () => {
                            console.log(
                              'Master Data Audit - Download New Data Block inserted successfully',
                            );
                          },
                        );
                      });
          ...
          if (Trees.length != 0) {
            db.transaction(tx => {
              tx.executeSql(
                ...
                (tx, results) => {
                  var len = results.rows.length;
                  let totalImages = results.rows.length;
                  if (len > 0) {
                    for (let i = 0; i < results.rows.length; ++i) {
                      let droneImagePath =
                      ...
                      const progress2 = ((i + 1) / totalImages) * 100;
  ...
  return (
    <View style={gStyle.container[theme]}>
      <View
        ...
        {progress === 100 && progress2 == 100 && downloadComplete ? (
          <StandardAlert
            visible={visible}
            color={'green'}
